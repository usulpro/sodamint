...
- Phase status: done (2026-07-18 — repo docs aligned to the dashboard model; whole-feature E2E passed against the reshaped desired outcome)
...
- [x] Kind: verification | Status: done | Whole-feature run mimicking the real workload: two parallel agent inhibitors (`--who=sodamint-agent`) plus the manual toggle; toggle off; externally kill one agent; quit.
  ...
  - Closed 2026-07-18: in-process AppIndicator run with a real `Gtk.main()` loop, agents spawned via the `AGENTS.md` one-liner form. All 4 steps PASS — step1: 2× `◆` (grouped first) + 1× `★` + `Awake — 10 sources`; step2: toggle off drops only our `★`, both `◆` remain, our lock released; step3: external kill of agent A → its `◆` gone within a poll (logind auto-cleanup), B remains; step4: real `quit()` → agent B still held in logind, our lock gone. No `waitid` warning; no product code changed; test locks cleaned up.
...
- [x] Kind: verification | Status: done | Install the built `.deb` end-to-end in a clean environment and confirm the packaged app runs and keeps-awake.
  ...
  - Closed 2026-07-18: fresh `ubuntu:24.04` Docker container. All 4 packaging criteria PASS — `apt install ./…deb` auto-resolved and installed the four deps (`python3-gi`, `gir1.2-gtk-3.0`, `gir1.2-ayatanaappindicator3-0.1`, `systemd`; no `universe` needed); `dpkg -L` file map correct + launcher executable; installed `.py` `py_compile`s and `import gi/Gio/GLib/Gtk` works; `apt remove` removes every file incl. `/usr/share/sodamint` and `purge` leaves no record. Runtime keep-awake is out of container scope (no live logind) — already host-verified in Phases 2–4. Note: a first-pass STRAY `/usr/share/sodamint` was a verification artifact (in-place `py_compile` wrote `__pycache__`); re-run with `PYTHONPYCACHEPREFIX` confirmed clean removal. No packaging defect; recipe unchanged.
- [x] Kind: implementation | Status: done | Prepare a GitHub Release for the `.deb` and document the user install path; the actual publish is a user-gated one-command step.
