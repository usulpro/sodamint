...
| Poll interval too slow → stale list; too fast → churn | Low: minor UX lag or wasted cycles | Modest `GLib.timeout` (a few seconds) + refresh on menu popup; single small D-Bus call is cheap | mitigated (Phase 2: `POLL_SECONDS=4` + popup refresh + change-signature guard) |
| AppIndicator menu is static; dynamic list won't update | Medium: tray shows stale rows | `_refresh()` rebuilds the section and re-applies via `set_menu()`; verify on both AppIndicator and StatusIcon | mitigated (Phase 2: full rebuild + `set_menu()`, live-verified on AppIndicator; both-backends visual check in task 3) |
| Existing `GLib.child_watch_add` + `stop()`'s `subprocess.wait()` double-reap → `waitid: No child processes` GLib warning | Low: harmless stderr warning; UI stays correct | Fixed in Phase 3 task 1: `stop()` `GLib.source_remove()`s the stored child-watch id before reaping; external-death detection preserved. Verified no warning + external SIGTERM still resets UI. | resolved |
