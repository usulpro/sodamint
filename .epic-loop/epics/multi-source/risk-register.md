...
| Existing `GLib.child_watch_add` + `stop()`'s `subprocess.wait()` double-reap → `waitid: No child processes` GLib warning | Low: harmless stderr warning; UI stays correct | Pre-existing in the unchanged lock lifecycle (not introduced by Phase 2); reconcile in Phase 3 when the toggle/lifecycle is revisited (e.g. drop the redundant `wait()` or the watch) | open |
